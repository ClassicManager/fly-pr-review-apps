...
buildArgs=$(echo "$INPUT_BUILDARG" | sed 's/\([^ ]*\)/--build-arg \1/g')
...
if [ -n "$INPUT_VM" ]; then
  flyctl deploy "$INPUT_BUILDARG" --build-target "$INPUT_BUILDTARGET" --dockerfile "$INPUT_DOCKERFILE" --config "$config" --app "$app" --regions "$region" --image "$image" --strategy immediate --ha=$INPUT_HA --vm-size "$INPUT_VMSIZE"
else
